...
const DEFAULT_CONFIG = {
  trialDays: 14,
  plans: [
    { id: 'free', name: 'Бесплатный', price: 0, carLimit: 1 },
...
function mergeDefaults(cfg) {
  ...
  // Тарифы хранятся на диске (data/config.json) и сохраняются целиком при
  // каждом сохранении настроек в /admin — поэтому раньше при появлении
  // нового тарифа в коде (DEFAULT_CONFIG.plans) он не попадал к уже
  // работающим серверам: сохранённый на диске массив просто целиком
  // подменял собой значение по умолчанию. Здесь вместо простой подмены —
  // слияние по id: сохранённые/отредактированные администратором тарифы
  // сохраняются как есть, а тарифы из DEFAULT_CONFIG, которых ещё нет на
  // диске (по id), добавляются в конец. Так новый тариф из обновления кода
  // всегда появляется, даже если /admin уже когда-то сохранял старый список.
  const savedPlans = Array.isArray(cfg && cfg.plans) ? cfg.plans : [];
  const savedIds = new Set(savedPlans.map(p => p && p.id));
  const missingDefaults = DEFAULT_CONFIG.plans.filter(p => !savedIds.has(p.id));
  merged.plans = savedPlans.length ? savedPlans.concat(missingDefaults) : DEFAULT_CONFIG.plans;
