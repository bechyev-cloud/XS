...
async function createRequest(userId, planId, months, contactPhone, clientName, paymentBank) {
  ...
  const req = {
    ...
    clientName: (clientName || '').trim(), // раунд 21: Ф.И.О. клиента, вписанное вручную в заявке — не обязательно совпадает с именем аккаунта
    paymentBank: (paymentBank || '').trim(), // раунд 21: банк/сервис, с которого клиент планирует перевести оплату
